...
namespace DS
{
...

    {
        ...
        public  void Remove(List<object> objList, object obj)
         {
             if ((objList.Exists(obj.Equals)))
                 objList.RemoveAt(objList.FindIndex(obj.Equals));
             else throw new Exception(obj + "is not exist");
         }

        public  void Update(List<object> objList, object obj)
         {
             if ((objList.Exists(obj.Equals)))
             {
                if (obj is Contract) // if the type is contract so the contract number should be updated
                ((Contract)obj).ContractNum = ((Contract)objList.Find(obj.Equals)).ContractNum;
                objList.RemoveAt(objList.FindIndex(obj.Equals));
                objList.Add(obj);
            }
             else throw new Exception(obj + "is not exist");
